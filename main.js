...
const pies = [
  ...
  const cardBuilder = (pieArr) => {
      ...
      for ( i = 0; i < pieArr.length; i++){
        const currPie = pieArr[i];  
//      domString += `<h1>${currPie.name}</h1>`
        domString += `
        <div class = "col-3">
        <div class="card">
        <img src="${currPie.imageUrl}" class="card-img-top" alt="...">
        <div class="card-body">
          <h5 class="card-title">${currPie.name}</h5>
          <p class="card-text">${currPie.instructor} loves ${currPie.iceCream}</p>
        </div>
        </div>
      </div>`;
      }
